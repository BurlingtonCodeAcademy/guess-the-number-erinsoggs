...
async function chooseGame() {
  ...
  if (parseInt(gameChoice) === 1) {
    ...
    console.log("\nYour answer is invalid! Please restart the game. :)");
    process.exit();
  }
}
...
async function start() {
  ...
  if (isNaN(maxNumber) || maxNumber === "") {
    console.log("\nYou didn't enter a number! Please restart the game!");
    process.exit();
  }
  ...
  if (isNaN(secretNumber)) {
    ...
    guess += 1;
    console.log(`\nI'm going to guess that your number is: ${computerGuess}`);
  }
  ...
  while (secretNumber !== computerGuess) {
    ...
    if (
      ...
      guess += 1;
      console.log(`\nMy next guess is: ${computerGuess}`);
...
async function reverseStart() {
  console.log(
    `\nExcellent game choice! I'm going to pick a number and you're going to guess it!\nThe number I've picked is between ${minNumber} and ${maxNumber}.`
  );
  ...
  while (humanGuess !== computerNumber) {
    ...
    if (humanGuess < computerNumber) {
      ...
      humanGuess = parseInt(humanGuess);
      if (humanGuess !== parseInt(humanGuess)) {
        console.log(
          "You either entered an invalid answer or are cheating (booooo!) Please restart the game!"
        );
        process.exit();
  ...
  if (humanGuess === computerNumber) {
    console.log("\nI'm a sore loser, but you win!!! Game over.");
    process.exit();
  }
}
